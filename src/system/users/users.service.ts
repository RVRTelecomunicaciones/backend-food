...
export class UsersService {
  ...
  async getUserById(id: number): Promise<UserPreviewDto> {
    ...
    if (!user) {
      throw new NotFoundException(`Healthy Dev le informa que el usuario con id "${id}" no fue encontrado`);
    }
    return user;
  }
  ...
  async updateUser(updateData: UpdateUserDto, username: string): Promise<{ message: string }> {
    if (updateData.profilePhoto) {
      ...
      try {
        updateData.profilePhoto = await this.imageManagementService.uploadImage(updateData.profilePhoto);
      } catch (error) {
        throw new InternalServerErrorException('Healthy Dev no pudo guardar nueva imagen de perfil y cancelo cambios');
      }
      if (profilePhoto) {
        try {
  ...
  async deleteUserByToken(tokenDto: TokenDto): Promise<{ message: string }> {
